...
export default function Admin() {
    const problem = [
    {type:`Create problem`,icon:Plus,color:"bg-teal-500",color1:"bg-green-400",des:`Add a new coding problem to the platform`},
    {type:`Update problem`,icon:Edit,color:"bg-stone-500",color1:"bg-yellow-400",des:`Edit existing problem and their details`},
    {type:`Delete problem`,icon:Trash2,color:"bg-rose-300",color1:"bg-rose-400",des:`Remove problem from the platform`},
    ]
    return (
     <>
     ...
     <div className="grid lg:grid-cols-3 mt-5 lg:mx-15 md:mx-10 sm:mx-6 md:grid-cols-2 sm:grid-cols-1 "> 
      {
        problem.map((p,index)=>(
         <div key={index} className="bg-gray-800 border-1  border-gray-900 rounded-lg m-2 p-3 flex flex-col items-center text-center">
           ...
           <button className={`mt-6 mb-4 rounded-lg p-2 px-2.5 cursor-pointer font-semibold text-black ${p.color1}`}>{p.type}</button>
         </div>
        )
        )
      }
